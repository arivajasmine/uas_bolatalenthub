                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table tablesorter" id="ipi-table">
                                        ...
                                        <tbody class="text-center">
                                            @if ($data->count() > 0)
                                            @foreach($data as $kompetisi)
                                            <tr>
                                                <td>{{ $kompetisi->ID_Kompetisi }}</td>
                                                <td>{{ $kompetisi->Nama_Kompetisi }}</td>
                                                <td>{{ $kompetisi->Tanggal_Mulai }}</td>
                                                <td>{{ $kompetisi->Tanggal_Berakhir }}</td>
                                                <td>{{ $kompetisi->Juara }}</td>
                                                <td class="text-center align-middle" style="max-height: 60px; height: 60px;">
                                                    <a class="btn btnMaterial btn-flat success semicircle" role="button" href="{{ route('kompetisi.edit', $kompetisi->ID_Kompetisi) }}"><i class="fas fa-pen"></i></a>
                                                    {{-- <a class="btn btnMaterial btn-flat success semicircle" role="button" href="{{ url('kompetisi/'.$item->ID_Kompetisi.'edit') }}"><i class="fas fa-pen"></i></a> --}}
                                                    {{-- <form action="{{ route('kompetisi.destroy', $kompetisi->ID_Kompetisi) }}" method="POST" onsubmit="return confirm('Delete?')"> --}}
                                                    <form action="{{ route('kompetisi.destroy', $kompetisi->ID_Kompetisi) }}" method="POST" onsubmit="return confirm('Delete?')">
                                                        @csrf
                                                        @method('DELETE')
