                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table tablesorter" id="ipi-table">
                                        ...
                                        <tbody class="text-center">
                                            @if ($data->count() > 0)
                                            @foreach($data as $stat_tim)
                                            <tr>
                                                <td>{{ $stat_tim->ID_Statistik_Tim }}</td>
                                                <td>{{ $stat_tim->ID_Tim }}</td>
                                                <td>{{ $stat_tim->Point }}</td>
                                                <td>{{ $stat_tim->Menang }}</td>
                                                <td>{{ $stat_tim->Seri }}</td>
                                                <td>{{ $stat_tim->Kalah }}</td>
                                                <td>{{ $stat_tim->Gol_Memasukkan }}</td>
                                                <td>{{ $stat_tim->Gol_Kemasukkan }}</td>
                                                <td>{{ $stat_tim->Musim }}</td>
                                                <td>{{ $stat_tim->ID_Kompetisi }}</td>
                                                <td class="text-center align-middle" style="max-height: 60px; height: 60px;">
                                                    <a class="btn btnMaterial btn-flat success semicircle" role="button" href="{{ route('stat_tim.edit', $stat_tim->ID_Statistik_Tim) }}"><i class="fas fa-pen"></i></a>
                                                    <form action="{{ route('stat_tim.destroy', $stat_tim->ID_Statistik_Tim) }}" method="POST" onsubmit="return confirm('Delete?')">
                                                        @csrf
                                                        @method('DELETE')
